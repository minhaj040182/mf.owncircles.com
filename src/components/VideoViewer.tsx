...
import { generateSlug, getVideoShareUrl } from '../utils/seo';
...
import { ProsConsVoting } from './ProsConsVoting';
import { CompareAlternatives } from './CompareAlternatives';
import { PriceDropPredictor } from './PriceDropPredictor';
...
export const VideoViewer: React.FC<VideoViewerProps> = ({ 
  ...
  const [pulseOption, setPulseOption] = useState<'overview' | 'ask_ai' | 'topics' | 'quotes' | 'faq' | 'checklist' | 'proscons' | 'buy_vs_skip' | 'sentiment' | 'price_comparison'>('overview');
  ...
  const pageUrl = getVideoShareUrl({ slug, title: video.title, rephrasedTitle: video.rephrasedTitle });
  const products = video.products || [];

  // Load comments from API on mount or video change and update SEO Meta tags
  useEffect(() => {
    // Scroll directly to the player so visitor immediately watches the video
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-16 animate-fadeIn">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            {(() => {
              ...
              return (
                <div 
                  ...
                  className="relative aspect-video rounded-2xl overflow-hidden bg-slate-900 border border-gray-200 shadow-xl group scroll-mt-24"
                >
                  <iframe
                    key={`yt_${activeYtId}_${seekTimestamp}`}
                    src={`https://www.youtube.com/embed/${activeYtId}?autoplay=1&start=${seekTimestamp}&rel=0&enablejsapi=1&playsinline=1`}
                    title={displayTitle}
            ...
            {/* 1. INTERACTIVE QUICK VERDICT & SCORECARD (ABOVE THE FOLD) */}
            <QuickScorecard
              pulse={pulse}
              product={products[0]}
              category={video.category}
              region={activeRegion}
              onComparePriceClick={() => setPulseOption('price_comparison')}
              onScrollToDecision={() => decisionRef.current?.scrollIntoView({ behavior: 'smooth' })}
              onAskAIClick={() => {
                setPulseOption('ask_ai');
                decisionRef.current?.scrollIntoView({ behavior: 'smooth' });
              }}
            />

            {/* 2. KEY VIDEO TIMESTAMPS & CHAPTER BOOKMARKS */}
            <VideoChapters
              onSeek={(seconds) => setSeekTimestamp(seconds)}
              activeSeconds={seekTimestamp}
              productName={products[0]?.name || displayTitle}
              category={video.category}
            />
            ...
            <div ref={decisionRef} id="buyer-decision-engine" className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2 bg-slate-100 p-1.5 rounded-xl text-xs font-bold">
                ...
                <button
                  onClick={() => setPulseOption('ask_ai')}
                  className={`px-3.5 py-1.5 rounded-lg transition-all flex items-center gap-1.5 cursor-pointer ${
                    pulseOption === 'ask_ai' ? 'bg-indigo-600 text-white shadow-xs' : 'text-indigo-700 bg-indigo-50/80 hover:bg-indigo-100'
                  }`}
                >
                  <Bot className="w-3.5 h-3.5" />
                  <span>Ask AI Concierge</span>
                  <span className={`text-[9px] font-black px-1.5 py-0.2 rounded-full uppercase ${
                    pulseOption === 'ask_ai' ? 'bg-white/20 text-white' : 'bg-indigo-200 text-indigo-900'
                  }`}>
                    Live AI
                  </span>
                </button>
              ...
              {pulseOption === 'price_comparison' && (
                <div className="space-y-6 animate-fadeIn">
                  <PriceDropPredictor
                    productName={products[0]?.name || displayTitle}
                    currentPrice={products[0]?.estimatedPrice || '4999'}
                    region={activeRegion}
                  />
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            {/* AI Price Drop Predictor in Sidebar */}
            <PriceDropPredictor
              productName={products[0]?.name || displayTitle}
              currentPrice={products[0]?.estimatedPrice || '4999'}
              region={activeRegion}
        ...
        {/* 5. COMPARE WITH TOP ALTERNATIVES CAROUSEL & COMPARISON */}
        <div className="pt-2">
          <CompareAlternatives
            currentProduct={products[0]}
            currentTitle={displayTitle}
            category={video.category || 'electronics'}
            region={activeRegion}
            allVideos={allVideos}
            onSelectAlternativeVideo={onSelectRelated}
          />
        </div>
            ...
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  ...
                  className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-blue-400 text-xs font-bold rounded-xl border border-slate-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                  title="Compare with other stores"
                >
                  <Building2 className="w-3.5 h-3.5" />
                  <span className="hidden md:inline">Compare Stores</span>
                </button>

                <button
                  onClick={() => handleCopyProductLink(primaryLocalUrl, primaryProduct?.id || 'sticky')}
                  className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                ...
                <a
                  ...
                  target="_blank"
                  rel="sponsored nofollow noopener noreferrer"
                  className="flex-1 sm:flex-initial px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-md active:scale-95 cursor-pointer"
                >
                  <ShoppingBag className="w-4 h-4" />
      ...
      <ShareModal
        ...
        description={video.pulse?.summary}
      />
    </div>
  );
};
