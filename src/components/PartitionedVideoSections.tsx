...
import { TodayMostViewsProduct } from './TodayMostViewsProduct';
...
export const PartitionedVideoSections: React.FC<PartitionedVideoSectionsProps> = ({
  ...
  // Helper to ensure each rail receives unique videos
  const deduplicateList = (items: VideoItem[]): VideoItem[] => {
  ...
  const strictlyMobilesAndLaptops = deduplicateList(
    [...videos]
      .filter(v => {
  ...
  const todayTrending = deduplicateList(
    [...videos].sort((a, b) => b.pulse.viralPotentialScore - a.pulse.viralPotentialScore)
  ).slice(0, 6);

  // 2. Today's Hot Viral Deals & Editor's Picks (sorted by product conversion potential & engagement)
  const hotDeals = deduplicateList(
    [...videos]
  ...
  const topRated = deduplicateList(
    [...videos].sort((a, b) => b.pulse.overallSentimentRatio.positive - a.pulse.overallSentimentRatio.positive)
  ).slice(0, 6);
  ...
  return (
    <div className="space-y-12 py-2">
      {/* SECTION: TODAY MOST VIEWS PRODUCT (Slider type, placed just above Trending Mobiles & Laptop, automatically hidden if 0 views) */}
      <TodayMostViewsProduct
      ...
      {strictlyMobilesAndLaptops.length > 0 && (
        <section className="space-y-4">
          ...
          >
            {strictlyMobilesAndLaptops.map((video, idx) => (
              <div key={`mobile-laptop-${video.id}-${idx}`} className="w-[300px] sm:w-[340px] shrink-0 snap-start flex flex-col">
                <VideoCard 
                  video={video} 
      ...
      <section className="space-y-4">
        ...
        <div 
          ...
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {todayTrending.map((video, idx) => (
            <div key={`trending-${video.id}-${idx}`} className="w-[300px] sm:w-[340px] shrink-0 snap-start flex flex-col">
              <VideoCard 
                video={video} 
                onSelect={onSelectVideo} 
                region={region} 
      ...
      <section className="space-y-4">
        ...
        <div 
          ...
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {hotDeals.map((video, idx) => (
            <div key={`hotdeal-${video.id}-${idx}`} className="w-[300px] sm:w-[340px] shrink-0 snap-start flex flex-col">
              <VideoCard 
                video={video} 
                onSelect={onSelectVideo} 
                region={region} 
      ...
      <section className="space-y-4">
        ...
        >
          {topRated.map((video, idx) => (
            <div key={`top-${video.id}-${idx}`} className="w-[300px] sm:w-[340px] shrink-0 snap-start flex flex-col">
              <VideoCard 
                video={video} 
                onSelect={onSelectVideo} 
                region={region} 
      ...
      <div className="space-y-10">
        ...
        {categoriesList.map(cat => {
          const catVideos = deduplicateList(videos.filter(v => isCategoryMatch(v.category, cat.id)));
          ...
          return (
            <div key={cat.id} className="space-y-4 bg-white p-5 sm:p-6 rounded-3xl border border-gray-200/80 shadow-2xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pt-1">
                {catVideos.slice(0, 3).map((video, idx) => (
                  <VideoCard 
                    key={`cat-${cat.id}-${video.id}-${idx}`} 
                    video={video} 
                    onSelect={onSelectVideo} 
                    region={region} 
                    isBookmarked={savedVideoIds.includes(video.id)}
        ...
        {onLoadMore && (
          <div className="flex flex-col items-center justify-center pt-6 pb-2 space-y-3">
            <div className="text-xs font-semibold text-slate-500">
              Total <span className="font-bold text-slate-800">{videos.length}</span> analyzed reviews loaded across all categories
            </div>
            <button
              onClick={onLoadMore}
              disabled={isLoadingMore}
              className="px-6 py-3 bg-white hover:bg-blue-50 text-blue-600 hover:text-blue-700 font-extrabold text-sm rounded-2xl border-2 border-blue-200 hover:border-blue-300 shadow-sm transition-all flex items-center gap-2.5 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed hover:scale-102 active:scale-98"
            >
              {isLoadingMore ? (
                <>
                  <RefreshCw className="w-4 h-4 animate-spin text-blue-600" />
                  <span>Fetching Next Batch from Database...</span>
                </>
              ) : (
                <>
                  <Layers className="w-4 h-4 text-blue-600" />
                  <span>Load More Product Reviews</span>
                </>
              )}
            </button>
          </div>
