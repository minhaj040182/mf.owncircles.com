...
export interface DeferredProductCardProps {
  id?: string;
  title?: string;
  slug?: string;
  category?: string;
...
const checkIsCrawler = (): boolean => {
  ...
  return botSignatures.some((sig) => ua.includes(sig)) || Boolean((window as unknown as { __PRERENDER_INJECTED?: boolean }).__PRERENDER_INJECTED);
};

export const DeferredProductCard: React.FC<DeferredProductCardProps> = ({
  id,
  ...
  const [isVisible, setIsVisible] = useState<boolean>(isBot);
  ...
  const priceDisplay = typeof price === 'number' ? `$${(price as number).toFixed(2)}` : String(price);
