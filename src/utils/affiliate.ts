...
function getSafeAmazonDomain(domain: string = 'amazon.in'): string {
  const clean = domain.replace(/^www\./, '').toLowerCase();
  return OFFICIAL_DOMAINS.has(clean) ? clean : 'amazon.com';
}
...
export function createAmazonSearchLink(query: string, domain: string = 'amazon.in', tag: string = AFFILIATE_ID): string {
  const safeDomain = getSafeAmazonDomain(domain);
  if (!query) return `https://www.${safeDomain}/s?k=trending+products&tag=${tag}`;
  ...
  return `https://www.${safeDomain}/s?k=${cleanTerms}&tag=${tag}`;
}
...
export function convertAmazonUrl(urlOrQuery: string, tag: string = AFFILIATE_ID, domain: string = 'amazon.in'): string {
  const safeDomain = getSafeAmazonDomain(domain);
  if (!urlOrQuery) return `https://www.${safeDomain}/s?k=trending+products&tag=${tag}`;
  ...
  if (asin) {
    return `https://www.${safeDomain}/dp/${asin}?tag=${tag}`;
  }
  ...
  if (!trimmed.startsWith('http://') && !trimmed.startsWith('https://') && !trimmed.includes('amazon.') && !trimmed.includes('amzn.to')) {
    return createAmazonSearchLink(trimmed, safeDomain, tag);
  }
  ...
  try {
    ...
    if (isAmazon) {
      ...
      if (pathAsin) {
        return `https://www.${safeDomain}/dp/${pathAsin}?tag=${tag}`;
      }

      parsedUrl.hostname = `www.${safeDomain}`;
      ...
      if (parsedUrl.pathname.includes('/s')) {
        ...
        if (queryTerm) {
          return createAmazonSearchLink(queryTerm, safeDomain, tag);
        }
      }
