...
export function rephraseTitleMeaningfully(originalTitle: string, category?: string): string {
  ...
  if (lower.includes('pet gadgets') || lower.includes('luxury amazon pet') || lower.includes('pet gadget')) {
    return 'Comprehensive Buyer Guide & Performance Test: 25 Best Luxury Amazon Pet Gadgets That Everyone Is Buying';
  }
...
const TOPICS: Record<Category, { title: string; ytId: string; price: string }[]> = {
  ...
  electronics: [
    ...
    { title: 'Smart Double Door Refrigerator & Inverter Compressor Review', ytId: 'dJ8fQ6vX2X0', price: '$899.00' },
...
export function generateTenReviewVideosBatch(
  ...
  const allTopicPool: {
    ...
    },
    {
      ...
      likeCount: '124.5K',
      commentCount: '8.9K',
      thumbnailUrl: 'https://i.ytimg.com/vi/sz-y6Gz6Ikg/hqdefault.jpg'
    },
    {
      ...
      likeCount: '58.2K',
      commentCount: '4.1K',
      thumbnailUrl: 'https://i.ytimg.com/vi/3I4X8aM8ySg/hqdefault.jpg'
    },
    {
      ...
      likeCount: '32.1K',
      commentCount: '2.4K',
      thumbnailUrl: 'https://i.ytimg.com/vi/v8_aE9jRz6A/hqdefault.jpg'
    },
    {
    ...
    },
    {
      ...
      likeCount: '95.2K',
      commentCount: '5.1K',
      thumbnailUrl: 'https://i.ytimg.com/vi/_SHe391XlJw/hqdefault.jpg'
    },
    {
      ...
      likeCount: '18.3K',
      commentCount: '1.1K',
      thumbnailUrl: 'https://i.ytimg.com/vi/dJ8fQ6vX2X0/hqdefault.jpg'
    },
    {
    ...
    {
      title: 'Comprehensive Buyer Guide & Performance Test: 25 Best Luxury Amazon Pet Gadgets That Everyone Is Buying',
      ytId: 'GMx4sXjtul8',
      price: '$149.99',
      category: 'pet_supplies',
      channelTitle: 'Pet Innovations & Reviews',
      viewCount: '280.4K',
      likeCount: '8.4K',
      commentCount: '645',
      thumbnailUrl: 'https://i.ytimg.com/vi/GMx4sXjtul8/hqdefault.jpg'
    ...
    // Household category reviews with real valid YouTube IDs
    {
      ...
      likeCount: '8.1K',
      commentCount: '1.1K',
      thumbnailUrl: 'https://i.ytimg.com/vi/P-YXqWLouB0/hqdefault.jpg'
    },
    {
    ...
    },
    {
      ...
      likeCount: '3.1K',
      commentCount: '410',
      thumbnailUrl: 'https://i.ytimg.com/vi/q9L_1S39E1Q/hqdefault.jpg'
    },
    {
      ...
      likeCount: '2.9K',
      commentCount: '305',
      thumbnailUrl: 'https://i.ytimg.com/vi/r5x-J8Xg7d8/hqdefault.jpg'
    },
    {
  ...
  const seenYtInBatch = new Set<string>();
  const maxItems = Math.min(10, pool.length);

  for (let i = 0; i < pool.length && selectedTen.length < maxItems; i++) {
    const itemIndex = (batchOffset * 10 + i) % pool.length;
    const t = pool[itemIndex];
    if (seenYtInBatch.has(t.ytId)) {
      continue;
    }
    seenYtInBatch.add(t.ytId);
...
/**
 * Generates an instant, highly detailed VideoItem on-the-fly for any valid product review slug.
 * Used to guarantee 100% crawl success (200 OK) for Google Search Console crawlers and users.
 */
export function generateVideoFromSlug(slug: string, region: RegionCode = 'IN'): VideoItem {
  const cleanSlug = slug.replace(/^video\//i, '').trim();
  
  // Extract meaningful product keywords from the slug
  const rawWords = cleanSlug
    .replace(/\b(comprehensive|buyer|guide|performance|test|unbiased|field|testing|hands|on|review|in|depth|usability|feature|breakdown|top|rated|durability|evaluation|assessment|value|essential|ergonomic|parent|usage|hygiene|automation|safety|setup|practical|must|watch|amazon|finds|unboxing|2026|2025|2024)\b/gi, ' ')
    .replace(/[^\w\s-]/g, ' ')
    .trim();

  const words = rawWords.split(/[\s_-]+/).filter(w => w.length > 0);
  const cleanTitle = words.length > 0
    ? words.map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')
    : cleanSlug.replace(/-/g, ' ');

  const title = cleanTitle.length > 3 ? cleanTitle : `Top-Rated Product Review`;
  const lower = cleanSlug.toLowerCase();

  // Infer category & appropriate YouTube video ID
  let category: Category = 'household';
  let ytId = 'bCXhRtb16mk';

  if (lower.includes('grinder') || lower.includes('spice') || lower.includes('blender') || lower.includes('kitchen') || lower.includes('cook') || lower.includes('pan') || lower.includes('pot') || lower.includes('fryer')) {
    category = 'kitchen';
    ytId = 'PRgy1nnm3fg';
  } else if (lower.includes('phone') || lower.includes('laptop') || lower.includes('tv') || lower.includes('pc') || lower.includes('tablet') || lower.includes('camera') || lower.includes('electronic')) {
    category = 'electronics';
    ytId = '2QkS5j6bH_g';
  } else if (lower.includes('treadmill') || lower.includes('dumbbell') || lower.includes('fitness') || lower.includes('walking') || lower.includes('workout') || lower.includes('gym')) {
    category = 'fitness';
    ytId = '1fbUlzz2zfY';
  } else if (lower.includes('power') || lower.includes('gadget') || lower.includes('anker') || lower.includes('charger') || lower.includes('smart')) {
    category = 'gadgets';
    ytId = 'VadYsrjOusY';
  } else if (lower.includes('kindle') || lower.includes('book') || lower.includes('pen') || lower.includes('desk')) {
    category = 'books_stationery';
    ytId = 'XZ0pMbshy3o';
  } else if (lower.includes('dyson') || lower.includes('hair') || lower.includes('airwrap') || lower.includes('skin') || lower.includes('care')) {
    category = 'personal_care';
    ytId = 'LDL_dhnQF8Y';
  } else if (lower.includes('carseat') || lower.includes('baby') || lower.includes('stroller') || lower.includes('doona')) {
    category = 'baby_parenting';
    ytId = 'tfIvPh3Q7UM';
  } else if (lower.includes('litter') || lower.includes('cat') || lower.includes('dog') || lower.includes('pet')) {
    category = 'pet_supplies';
    ytId = 'GMx4sXjtul8';
  } else if (lower.includes('chair') || lower.includes('aeron') || lower.includes('office') || lower.includes('herman')) {
    category = 'home_office';
    ytId = 'gi9R57g9s9U';
  } else if (lower.includes('cooler') || lower.includes('yeti') || lower.includes('travel') || lower.includes('outdoor')) {
    category = 'travel_outdoor';
    ytId = 'S-N0wGozc1U';
  }
  ...
  const comments = [
    `I bought the ${title} from Amazon link https://www.${domain}/s?k=${encodeURIComponent(title)}&tag=${tag} and it exceeded expectations!`,
    `Great build quality and real-world durability. Highly recommended review.`
  ];
  ...
  videoItem.slug = cleanSlug;
  videoItem.region = region;
  videoItem.category = category;
  videoItem.thumbnailUrl = `https://i.ytimg.com/vi/${ytId}/hqdefault.jpg`;
  videoItem.products[0].name = title;
  videoItem.products[0].category = category;
  videoItem.products[0].estimatedPrice = formatPriceForRegion('$89.99', region);
  videoItem.products[0].affiliateUrl = getLocalizedAffiliateUrl(undefined, title, region);
  videoItem.products[0].affiliateTag = tag;
