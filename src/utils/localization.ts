...
export const OFFICIAL_AMAZON_DOMAINS = new Set([
  'amazon.com',
  'amazon.in',
  'amazon.co.uk',
  'amazon.ca',
...
export const REGION_CONFIGS: Record<RegionCode, RegionConfig> = {
  ...
  PK: {
    ...
    storeName: 'Amazon Global (Pakistan Delivery)',
  ...
  BD: {
    ...
    storeName: 'Amazon Global (Bangladesh Delivery)',
    deliveryNote: 'International Shipping & Import Duty Handled',
  },
  US: {
    code: 'US',
...
export function getLocalizedAffiliateUrl(
  ...
  const config = REGION_CONFIGS[region] || REGION_CONFIGS.IN;
  const safeDomain = OFFICIAL_AMAZON_DOMAINS.has(config.amazonDomain) ? config.amazonDomain : 'amazon.com';
  const safeTag = config.amazonTag || 'trends0628-21';
  ...
  if (originalUrl && originalUrl.includes('amazon') && originalUrl.includes(safeDomain)) {
    let updated = originalUrl;
    if (updated.includes('tag=')) {
      updated = updated.replace(/tag=[^&]+/g, `tag=${safeTag}`);
    } else {
      updated += (updated.includes('?') ? '&' : '?') + `tag=${safeTag}`;
    }
    return updated;
  }

  // Guaranteed 100% working live Amazon search link with Affiliate Tag for verified Amazon domain
  return `https://www.${safeDomain}/s?k=${query}&tag=${safeTag}`;
}
