...
// Helper function to create clean, SEO-friendly concise URL slugs (3-6 core keywords, under 45 chars)
function generateSeoSlug($text) {
    if (empty($text)) return 'trending-product-review';

    // 1. Strip boilerplate SEO prefixes that make URL excessively long
    $clean = preg_replace('/\b(Comprehensive Buyer Guide|Performance Test|Unbiased Field Testing|Hands-On Review|In-Depth Usability|Feature Breakdown|Top-Rated Product Review|Durability Evaluation|Field-Tested Assessment|Comprehensive Usability|Value Evaluation|Essential Ergonomic|Parent Usage Assessment|Hygiene & Automation Assessment|Safety Setup|Practical)\b/i', ' ', $text);
    $clean = preg_replace('/\b(MUST WATCH|AMAZON FINDS|UNBOXING|2026|2025|2024|WOW|OMG|SHOCKING|OFFICIAL|FULL REVIEW)\b/i', ' ', $clean);
    ...
    $stopwords = [
        ...
        'from', 'as', 'is', 'are', 'was', 'were', 'that', 'this', 'these', 'those', 'it', 'its',
        'you', 'your', 'we', 'our', 'what', 'which', 'who', 'how', 'when', 'where', 'why'
    ];
    $stopMap = array_flip($stopwords);

    $rawWords = preg_split('/[\s_-]+/', $clean, -1, PREG_SPLIT_NO_EMPTY);
    $words = [];
    ...
    // 4. Select top 4-6 keywords up to 45 chars max
    $selected = [];
    $currLen = 0;
    foreach ($words as $w) {
        if (count($selected) >= 6) break;
        if ($currLen + strlen($w) + (count($selected) > 0 ? 1 : 0) > 45) {
            if (empty($selected)) {
                $selected[] = substr($w, 0, 45);
            }
            break;
        }
        $selected[] = $w;
        $currLen += strlen($w) + (count($selected) > 1 ? 1 : 0);
    }

    $slug = implode('-', $selected);
    $slug = trim(preg_replace('/-+/', '-', $slug), '-');
    return !empty($slug) ? $slug : 'video-review-' . substr(md5($text), 0, 8);
}
...
function getAmazonConfigForRegion($region) {
    ...
    switch ($region) {
        case 'IN':
            return ['domain' => 'amazon.in', 'tag' => 'trends0628-21', 'currency' => '₹', 'baseUrl' => 'https://www.amazon.in/s?k='];
        case 'PK':
